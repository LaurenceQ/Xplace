...
class IOParser(object):
    ...
    def preprocess_design_info(
        self,
        gpdb,
        timing_only: bool = False,
        include_names: bool = True,
        include_celltype_names: bool = True,
    ):
        ...
        net_names = gpdb.net_names() if include_names else []
        pin_names = gpdb.pin_names() if include_names else []
        node_names = gpdb.node_names() if include_names else []
        ...
        node_id2celltype_name = (
            gpdb.node_id2celltype_name() if include_celltype_names else None
        )
        all_node_types = []
        mov_end_idx = None
        fix_end_idx = None
        ...
        design_info = {
            ...
            "node_lpos": node_lpos.contiguous() if node_lpos is not None else None,
            "node_size": node_size.contiguous(),
            "pin_rel_cpos": pin_rel_cpos.contiguous() if pin_rel_cpos is not None else None,
            "pin_rel_lpos": pin_rel_lpos.contiguous(),
            "pin_size": pin_size.contiguous(),
            "pin_id2node_id": pin_id2node_id.long().contiguous(),
            "pin_id2net_id": pin_id2net_id,
            "hyperedge_index": None if timing_only else hyperedge_index.long().contiguous(),
            "hyperedge_list": hyperedge_list.long().contiguous(),
            "hyperedge_list_end": hyperedge_list_end.long().contiguous(),
            "node2pin_index": None if timing_only else node2pin_index.long().contiguous(),
            "node2pin_list": node2pin_list.long().contiguous(),
            "node2pin_list_end": node2pin_list_end.long().contiguous(),
            "node_id2region_id": (
                node_id2region_id.long().contiguous() if node_id2region_id is not None else None
            ),
            "region_boxes": region_boxes.contiguous() if region_boxes is not None else None,
            "region_boxes_end": (
                region_boxes_end.long().contiguous() if region_boxes_end is not None else None
            ),
        }
        return design_info
