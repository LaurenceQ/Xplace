...
def load_dataset(args, logger, params):
    ...
    if args.load_from_raw:
        ...
        debug_needs_pin_names = bool(
            ...
            or getattr(args, "debug_dump_endpoint_tests", "")
            or getattr(args, "debug_endpoint_test_pin", None)
            or getattr(args, "debug_dump_endpoint_slacks", "")
            or getattr(args, "debug_report_k_path", 0)
        )
        include_names = (not direct_timing_only) or debug_needs_pin_names
        include_celltype_names = (not direct_timing_only) or getattr(args, "timing_opt", False)
...
class PlaceData(object):
    def __init__(
        ...
        **kwargs,
        ...
        self.pin_id2net_id = pin_id2net_id if pin_id2net_id is not None else hyperedge_index[1]
        ...
        num_nodes_for_special = node_pos.shape[0]
        if node_id2celltype_name is not None:
            num_nodes_for_special = len(node_id2celltype_name)
        self.node_special_type = torch.zeros(num_nodes_for_special, dtype=torch.int32)
        if getattr(args, "timing_opt", False) and node_id2celltype_name is not None:
    ...
    def to_timing_device(self, device):
        self.device = device
        return self.to(
            device,
            "die_info",
    ...
    def prescale_by_site_width_timing(self):
        scalar_at = torch.tensor([self.site_width], dtype=torch.float32, device=self.die_info.device)
        self.die_info /= scalar_at
        self.node_pos /= scalar_at
        self.node_size /= scalar_at
        if self.pin_rel_cpos is not None:
            self.pin_rel_cpos /= scalar_at
        self.pin_rel_lpos /= scalar_at
        self.pin_size /= scalar_at
        self.__die_scale__ *= scalar_at
    ...
    def pre_compute_timing_var(self):
        args = self.__args__
        ...
        self.net_weight = torch.ones(self.num_nets, device=device, dtype=dtype)
        self.__num_macros__ = 0
        self.__num_movable_macros__ = 0
        self.__num_fixed_macros__ = 0
        self.__total_mov_area_without_filler__ = 0.0
        self.__bin_area__ = torch.prod(self.unit_len).item()
        return self
    ...
    def logging_timing_statistics(self):
        args = self.__args__
        logger = self.__logger__
        content = "\n===================\n"
        content += "#nodes = %d, #nets = %d, #pins = %d\n" % (
    ...
    def preprocess_timing(self):
        self.init_transform_state()
        self.preshift_timing()
        self.prescale_by_site_width_timing()
        self.pre_compute_timing_var()
        self.logging_timing_statistics()
        return self
