...
void GPUTimer::update_rc_timing(torch::Tensor node_lpos, bool record, bool load, bool conpensation) {
    ...
    model.x = x;
    model.y = y;
    model.pin_offset_x = pin_offset_x;
    model.pin_offset_y = pin_offset_y;
    model.pin2node_map = pin2node_map;
    model.flat_net2pin_start_map = flat_net2pin_start_map;
    model.flat_net2pin_map = flat_net2pin_map;
    model.pinLoad = pinLoad;
    model.pinImpulse = pinImpulse;
    model.pinCap = pinCap;
    model.pinWireCap = pinWireCap;
    model.pinRootDelay = pinRootDelay;
    model.pinRootRes = pinRootRes;
    model.num_nets = num_nets;
    model.num_pins = num_pins;
    model.unit_to_micron = unit_to_micron;
    model.net_is_clock = net_is_clock;
    model.cf = cf;
    model.rf = rf;
    update_rc_timing_cuda(model);
    if (record) {
...
void GPUTimer::update_rc_timing_flute(torch::Tensor node_lpos, bool record) {
    ...
    rc_model.edge_from = &edge_from;
    rc_model.edge_to = &edge_to;
    rc_model.flat_net2node_start_map = &flat_net2node_start_map;
    rc_model.flat_net2edge_start_map = &flat_net2edge_start_map;
    rc_model.node2pin_map = &node2pin_map;
    rc_model.edge_wl = &edge_wl;
    rc_model.includes_pin_caps = &no_includes_pin_caps;
    ...
    rc_model.net_is_clock = net_is_clock;
    rc_model.num_nets = num_nets;
    rc_model.num_pins = num_pins;
    rc_model.num_nodes = num_nodes;
    rc_model.num_edges = num_edges;
    rc_model.unit_to_micron = unit_to_micron;
    rc_model.rf = rf;
    rc_model.cf = cf;

    calc_res_cap(rc_model);
