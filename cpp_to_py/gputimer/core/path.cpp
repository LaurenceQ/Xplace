...
tuple<vector<int64_t>, vector<float>, vector<float>> GPUTimer::report_path(int ep_idx, int el, bool verbose) {
    ...
    if (ep_idx == -1) {
        ...
        worst_ep_idx = ep_idx;
        auto ep_slack = torch::nan_to_num(pin_slacks[worst_ep_idx], FLT_MAX);
        if (el != -1) worst_ep_i = torch::argmin(ep_slack.index({"...", torch::indexing::Slice(2 * el, 2 * (el + 1))})).item<int>() + el * 2;
        else worst_ep_i = torch::argmin(ep_slack).item<int>();
    }
...
vector<vector<int64_t>> GPUTimer::report_K_path(int K, int el, bool verbose) {
    if (!pin_slacks.numel()) {
        pin_slacks = torch::zeros_like(timing_raw_db.pinAT, torch::dtype(torch::kFloat32).device(timing_raw_db.pinAT.device()));
        auto s1 = timing_raw_db.pinAT - timing_raw_db.pinRAT;
        auto s2 = timing_raw_db.pinRAT - timing_raw_db.pinAT;
    ...
    for (int i = 0; i < K; i++) {
        auto [path, path_at, to_delay] = report_path(endpoints_id[indices[i]].item<int>(), el, verbose);
