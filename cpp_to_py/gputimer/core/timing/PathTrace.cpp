...
// Report timing paths
//
tuple<vector<int64_t>, vector<float>, vector<float>> GPUTimer::report_path(int ep_idx, int el, int rf, bool verbose) {
    if (!pin_slacks.defined() || !pin_slacks.numel()) {
    ...
    // to_i encodes: el = ti>>1, rf = ti&1  (0=early-rise, 1=early-fall, 2=late-rise, 3=late-fall)
    auto invalid_ti = [&](int ti) {
        return (el != -1 && (ti >> 1) != el) || (rf != -1 && (ti & 1) != rf);
    };
    int worst_ep_idx;
    int worst_ep_i;
    if (ep_idx == -1) {
        ...
        for (int ti = 0; ti < 4; ti++)
            if (invalid_ti(ti)) ep_slacks.select(1, ti).fill_(FLT_MAX);
        ...
        worst_ep_idx = ep_idx;
        auto ep_slack = torch::nan_to_num(pin_slacks[worst_ep_idx], FLT_MAX).clone();
        for (int ti = 0; ti < 4; ti++)
            if (invalid_ti(ti)) ep_slack.index_put_({ti}, FLT_MAX);
        worst_ep_i = torch::argmin(ep_slack).item<int>();
    }
    ...
    vector<float> path_slew;   // 新增：存储pinSlew
    vector<float> path_load;   // 新增：存储pinLoad
    vector<float> path_gt_at;
    const bool has_gt_at = timing_raw_db.pinGT_AT.defined() &&
                           timing_raw_db.pinGT_AT.numel() == timing_raw_db.pinAT.numel();
    while (cur != -1) {
        ...
        float delay = 0;
        float slack = 0;
        float slew = 0;   // 新增：临时存储slew
        float load = 0;   // 新增：临时存储load
        float gt_at = nanf("");
        if (prev != -1) {
            if (has_gt_at) {
                gt_at = timing_raw_db.pinGT_AT[cur][to_i].item<float>();
            }
            ...
            slack = pin_slacks[cur][to_i].item<float>();
                // 始终获取slew和load（即使起点）
            slew = timing_raw_db.pinSlew[cur][to_i].item<float>();  // 从timing_raw_db获取
            load = timing_raw_db.pinLoad[cur][to_i].item<float>();  // 从timing_raw_db获取
        }

        path_gt_at.push_back(gt_at);
        path.push_back(cur);
        path_at.push_back(at);
        path_rat.push_back(rat);
        to_delay.push_back(delay);
        path_slack.push_back(slack);
        path_slew.push_back(slew);   // 存储slew
        path_load.push_back(load);   // 存储load
    ...
    if (verbose) {
        ...
        cout << '\n'
            << std::setw(10) << "Type"
            << std::setw(10) << "Load"    // 新增列
            << std::setw(10) << "Slew"    // 新增列
            << std::setw(10) << "Delay"
            << std::setw(10) << "AT"
            << std::setw(20) << "RAT"
            << std::setw(10) << "Slack"
        ...
        for (int i = path.size() - 1; i >= 0; i--) {
            int cur = path[i];
            cout << setw(10) << "pin "
                << setw(10) << path_load[i] // 输出load
                << setw(10) << path_slew[i]  // 输出slew
                << setw(10) << to_delay[i]
                << setw(10) << path_at[i] << "(" << path_gt_at[i] << ")"
                << setw(15) << path_rat[i]
                << setw(10) << path_slack[i];
...
vector<vector<int64_t>> GPUTimer::report_K_path(int K, int el, int rf, bool verbose) {
    if (!pin_slacks.defined() || !pin_slacks.numel()) {
        pin_slacks = torch::zeros_like(timing_raw_db.pinAT, torch::dtype(torch::kFloat32).device(timing_raw_db.pinAT.device()));
        auto s1 = timing_raw_db.pinAT - timing_raw_db.pinRAT;
        auto s2 = timing_raw_db.pinRAT - timing_raw_db.pinAT;
    ...
    auto ep_slacks = torch::nan_to_num(pin_slacks.index_select(0, endpoints_id), FLT_MAX);
    for (int ti = 0; ti < 4; ti++)
        if ((el != -1 && (ti >> 1) != el) || (rf != -1 && (ti & 1) != rf))
            ep_slacks.select(1, ti).fill_(FLT_MAX);

    auto [ep_slack_elw, order] = torch::min(ep_slacks, 1);
    ...
    for (int i = 0; i < K; i++) {
        auto [path, path_at, to_delay] = report_path(endpoints_id[indices[i]].item<int>(), el, rf, verbose);
