...
using PathIndexAccessor = torch::PackedTensorAccessor32<int64_t, 1, torch::RestrictPtrTraits>;
using PathIntAccessor = torch::PackedTensorAccessor32<int, 1, torch::RestrictPtrTraits>;

struct PathTraceModel {
    ...
    PathIntAccessor endpoints_index;
    float* from_pin_delay = nullptr;
    unsigned long long* from_pin_delay_uint64 = nullptr;
    PathIntAccessor pin_visited;
    int K = 0;
    unsigned long long scalar = 1;
};

__global__ void explore_path_kernel(PathTraceModel* model) {
    const int k = blockIdx.x * blockDim.x + threadIdx.x;
    if (k < model->K) {
        index_type cur_id = model->endpoints_index[model->indices[k]];
        int to_i = model->ep_i_indices[model->indices[k]];
        while (cur_id != -1) {
            atomicAdd(&model->pin_visited[cur_id], 1);
            int prev_id = model->at_prefix_pin[cur_id * NUM_ATTR + to_i];
            int arc_id = model->at_prefix_arc[cur_id * NUM_ATTR + to_i];
            int from_i = model->at_prefix_attr[cur_id * NUM_ATTR + to_i];
            int arc_i = (from_i << 1) + (to_i & 0b1);
            float delay = 0;
            if (prev_id != -1) {
                delay = model->arcDelay[arc_id * 2 * NUM_ATTR + arc_i] / pow(1 + k, 2);

                if (model->arc_types[arc_id] == 0) {
                    atomicAdd(&model->from_pin_delay[cur_id], delay);
                } else {
                    atomicAdd(&model->from_pin_delay[prev_id], delay);
                }
            }
            cur_id = prev_id;
            to_i = from_i;
...
__global__ void explore_path_deterministic_kernel(PathTraceModel* model) {
    const int k = blockIdx.x * blockDim.x + threadIdx.x;
    if (k < model->K) {
        index_type cur_id = model->endpoints_index[model->indices[k]];
        int to_i = model->ep_i_indices[model->indices[k]];
        while (cur_id != -1) {
            ...
            if (prev_id != -1) {
                ...
                if (model->arc_types[arc_id] == 0) {
                    atomicAdd(&model->from_pin_delay_uint64[cur_id], static_cast<unsigned long long>(delay * model->scalar));
                } else {
                    atomicAdd(&model->from_pin_delay_uint64[prev_id], static_cast<unsigned long long>(delay * model->scalar));
                }
            }
            cur_id = prev_id;
            to_i = from_i;
...
std::tuple<torch::Tensor, torch::Tensor> explore_path(index_type* at_prefix_pin,
    ...
    PathTraceModel* d_model = nullptr;
    cudaMalloc(&d_model, sizeof(PathTraceModel));
    ...
    if (deterministic) {
        ...
        model.from_pin_delay_uint64 = aux_array_uint64_ptr;
        model.scalar = scalar;
        cudaMemcpyAsync(d_model, &model, sizeof(PathTraceModel), cudaMemcpyHostToDevice, stream);
        explore_path_deterministic_kernel<<<numBlocks, numThreads, 0, stream>>>(d_model);
        copyToFloatAuxArray<<<cp_blocks, cp_threads, 0, stream>>>(
            aux_array_uint64_ptr, from_pin_delay.data_ptr<float>(), scalar, inv_scalar, num_pins);

    } else {
        cudaMemcpyAsync(d_model, &model, sizeof(PathTraceModel), cudaMemcpyHostToDevice, stream);
        explore_path_kernel<<<numBlocks, numThreads, 0, stream>>>(d_model);
    }
    cudaFree(d_model);
