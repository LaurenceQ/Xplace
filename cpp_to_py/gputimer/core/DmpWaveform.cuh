...
__device__ __forceinline__ double dmp_model::y0(double t, double rd, double cl) {
    return t - rd * cl * (1.0 - exp2(-t / (rd * cl)));
}

__device__ __forceinline__ double dmp_model::y(double t,
                                               double t0,
                                               double dt,
                                               double rd,
                                               double cl) {
...
__device__ __forceinline__ double dmp_model::y0dt(double t, double rd, double cl) {
    return 1.0 - exp2(-t / (rd * cl));
}

__device__ __forceinline__ double dmp_model::y0dcl(double t, double rd, double cl) {
    return rd * ((1.0 + t / (rd * cl)) * exp2(-t / (rd * cl)) - 1);
}

__device__ __forceinline__ void dmp_model::dy(double t,
                                              double t0,
                                              double dt,
                                              double rd,
                                              double cl,
