...
HostRcGraph GPUTimer::build_openroad_route_segments_rc(const std::string& file) {
    ...
    while (std::getline(input, line)) {
        ...
        if (one_token) {
            ...
            current_net = resolve_route_net_token(net_name_to_index, first, token_end);
        ...
        const int layer1 = resolve_route_layer_token(layer_name_to_level, layer1_begin, layer1_end);
        const int layer2 = resolve_route_layer_token(layer_name_to_level, layer2_begin, layer2_end);
        ...
        const int from = append_route_node(current_net, key1, local_nets, route_node_maps, keep_route_node_names);
        const int to = append_route_node(current_net, key2, local_nets, route_node_maps, keep_route_node_names);
        if (from == to) {
