...
void GTDatabase::readSdc(sdc::SDC& sdc) {
    ...
    for (auto& [clock_name, clock] : clocks) {
        ...
        pin_clock_ids[clock.source_id()] = intern_clock_period(clock.period());
    ...
    for (int test_id = 0; test_id < static_cast<int>(test_id2_arc_id.size()); ++test_id) {
        ...
        const Clock* clock = iter->second;
        test_clock_ids[test_id] = intern_clock_period(clock->period());
    ...
    for (int pin_id = 0; pin_id < num_pins; ++pin_id) {
        ...
        if (std::isfinite(rise_edge)) {
            timing_raw_db.pinAT[pin_id][0] = rise_edge;
            timing_raw_db.pinAT[pin_id][2] = rise_edge;
        }
        if (std::isfinite(fall_edge)) {
            timing_raw_db.pinAT[pin_id][1] = fall_edge;
            timing_raw_db.pinAT[pin_id][3] = fall_edge;
        }
    }
