 ...
 *     "labels":   [<4 floats>]   // wire delay [er,ef,lr,lf] in ns
 *   }
 ...
 *     "labels":   [<4 floats>]   // same wire delay [er,ef,lr,lf] in ps
 *   }
 *
 ...
 *     "labels":   [<4 floats>],             // arc delays per output transition [ps]
...
void GPUTimer::dump_timing_graph(const std::string& outfile) {
    ...
    // ----------------------------------------------------------------
    // 7.  Collect cell_out edges keyed by (from, to, timing_arc_id).
    //     Each unique timing_arc_id within a cell is one output edge.
    //     Early/late arc_ids that share the same timing_arc_id merge
    //     into the same edge's labels [er, ef, lr, lf].
    // ----------------------------------------------------------------
    ...
    struct CellOutKey {
        int from, to;
        TimingArc* arc;  // pointer to the underlying TimingArc

        bool operator==(const CellOutKey& o) const {
            return from == o.from && to == o.to && arc == o.arc;
        }
    };
    ...
    for (int arc_id = 0; arc_id < num_arcs; ++arc_id) {
        ...
        if (atype == 0) {
            ...
            // Build delay string once, shared by both directions
            ...
            // ---- cell arc: collect per TimingArc pointer for merging ----
            ...
            // Key merges both tid_e and tid_l if they point to the same TimingArc*.
            ...
            if (tid_e != -1) {
                ...
                auto& edge = cell_out_map[{from, to, arc_ptr}];
    ...
    // Each unique (from, to, TimingArc*) produces one edge
    // Skip constraint arcs (setup/hold)
    for (const auto& [key, edge] : cell_out_map) {
        if (key.arc && key.arc->is_constraint()) continue;

        std::string rec = "{\"type\":\"cell_out\",\"from\":" + std::to_string(key.from)
             + ",\"to\":" + std::to_string(key.to)
             + ",\"sdf_cond\":\"" + (key.arc ? key.arc->sdf_cond_ : "") + "\""
             + ",\"labels\":["
             ...
             + emit_lut(key.arc ? key.arc->cell_delay_[0] : nullptr)
             + ",\"lut_cell_delay_fall\":"
             + emit_lut(key.arc ? key.arc->cell_delay_[1] : nullptr)
             + ",\"lut_trans_rise\":"
             + emit_lut(key.arc ? key.arc->transition_[0] : nullptr)
             + ",\"lut_trans_fall\":"
             + emit_lut(key.arc ? key.arc->transition_[1] : nullptr)
             + "}}\n";
        fout << rec;
    }
